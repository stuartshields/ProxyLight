...
- CLI signal handling: `Start.run()` blocks the main thread on a self-pipe (`waitForShutdownSignal()`), with plain C `signal()` handlers for SIGINT/SIGTERM writing one byte to it. Don't switch back to `DispatchSource.makeSignalSource`: on Linux libdispatch backs it with `signalfd`, which never sees a signal set to `SIG_IGN`, so the macOS `SIG_IGN` + `DispatchSource` recipe silently ignores Ctrl-C and `kill` there.
